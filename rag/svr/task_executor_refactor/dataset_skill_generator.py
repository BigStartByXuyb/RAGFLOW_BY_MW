...
async def run_corpus2skill(
    ...
    # Per-document summaries use the chat model configured on the document's
    # compilation template, falling back to the tenant default. The first
    # document's model deterministically owns KB-wide cluster summaries and
    # labels.
    ...
    for doc in eligible_docs:
        doc_id = str(doc["id"])
        llm_id = str((doc.get("parser_config") or {}).get("llm_id") or "").strip()
        doc_llm_ids[doc_id] = llm_id
        if llm_id in chat_models:
            continue
        chat_model_config = resolve_model_config(ctx.tenant_id, LLMType.CHAT, llm_id) if llm_id else get_tenant_default_model_by_type(ctx.tenant_id, LLMType.CHAT)
