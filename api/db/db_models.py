...
def migrate_db():
    ...
    # DataFlow pipeline surfaces were removed once and are now supported
    # again; re-add the log columns for databases that already dropped them.
    alter_db_add_column(migrator, "pipeline_operation_log", "pipeline_id", CharField(max_length=32, null=True, help_text="Pipeline ID", index=True))
    alter_db_add_column(migrator, "pipeline_operation_log", "pipeline_title", CharField(max_length=32, null=True, help_text="Pipeline title", index=True))
    alter_db_add_column(migrator, "pipeline_operation_log", "dsl", JSONField(null=True, default=dict))
