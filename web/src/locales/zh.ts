...
`,
    ...
    knowledgeConfiguration: {
      ...
      parserTip: {
        naive: '按版面切分成固定大小的文本块，小段落会不断合并直到达到 token 阈值。',
        general: '按版面切分成固定大小的文本块，小段落会不断合并直到达到 token 阈值。',
        qa: '文件中每一对问答会切分为一个独立的文本块，支持 XLSX 或以 TAB 分隔的 CSV/TXT 文件。',
        resume: '将简历解析为结构化的候选人信息字段，而不是普通文本块。',
        manual: '按最底层的章节标题切分，每个章节（含图表）作为一个文本块。',
        table: '表格的每一行作为一个文本块，第一行须为列标题。',
        paper: '按论文章节（摘要、1.1、1.2 等）切分，便于分章节做摘要。',
        book: '按章节标题切分；PDF 请设置页码范围以跳过前言、附录等无关内容。',
        laws: '按法律条文级别切分，每条法规作为一个独立且完整的文本块。',
        presentation: '每一页幻灯片作为一个文本块，并保留该页缩略图。',
        picture: '通过 OCR 从图片中提取文字，OCR 效果不足时改用视觉大模型生成图片描述。',
        one: '将整篇文档作为单一文本块，适合对短文档做整体摘要。',
        audio: '将音频转写为文字后再切分转写文本。',
        email: '将邮件解析为结构化字段（主题、发件人、正文、附件），而不是普通文本块。',
        tag: '从"描述-标签"配对中构建标签集，用于给其他知识库的文本块打标签，本身不参与检索。',
      },
      upload: '上传',
      english: '英文',
      chinese: '中文',
