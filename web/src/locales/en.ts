...
`,
    ...
    knowledgeConfiguration: {
      ...
      parserTip: {
        naive:
          'Splits by layout into fixed-size chunks, merging small segments until the token threshold is reached.',
        general:
          'Splits by layout into fixed-size chunks, merging small segments until the token threshold is reached.',
        qa: 'Each question-answer pair in the file becomes one chunk, for XLSX or TAB-delimited CSV/TXT files.',
        resume: 'Parses a résumé into structured candidate fields instead of plain text chunks.',
        manual: 'Splits by the lowest-level section headings, keeping each section (with its figures and tables) as one chunk.',
        table: 'Treats every row of a table as one chunk; the first row must be the column headers.',
        paper: 'Splits an academic paper by section (abstract, 1.1, 1.2, ...) for section-aware summarization.',
        book: 'Splits by chapter/section headings; set page ranges for PDFs to skip front/back matter.',
        laws: 'Splits at the article level, keeping each legal article as one self-contained chunk.',
        presentation: 'Treats each slide as one chunk, including its thumbnail image.',
        picture: 'Extracts text from images via OCR, falling back to a vision LLM description when OCR text is insufficient.',
        one: 'Treats the entire document as a single chunk, for summarizing short documents as a whole.',
        audio: 'Transcribes audio to text and chunks the transcript.',
        email: 'Parses an email into structured fields (subject, sender, body, attachments) instead of plain chunks.',
        tag: 'Builds a tag set from description-tag pairs, used to tag chunks in other datasets rather than for retrieval.',
      },
      save: 'Save',
      me: 'Only me',
      team: 'Team',
