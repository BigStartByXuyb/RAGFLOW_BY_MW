...
/**
 * Parser setups exactly as saved, keyed by file format. Accepts either a
 ...
 * parser change). Returns null when parser_config carries no setups — type
...
export function getSavedParserSetups(
  ...
  const setups = parserConfig?.setups;
  if (!setups || typeof setups !== 'object') {
    return null;
  }
  return Object.values(setups) as ParserSetup[];
}
