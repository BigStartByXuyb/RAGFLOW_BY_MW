...
export const useDatasetGenerate = () => {
  ...
  const { mutateAsync: pauseGenerate } = useMutation({
    mutationKey: [DatasetKey.pauseGenerate],
    mutationFn: async ({ type }: { task_id: string; type: GenerateType }) => {
      ...
      // The task-cancel endpoint left with the DataFlow removal, so dataset
      // compilation cannot be paused; report it instead of faking success.
      throw new Error(t('message.compileNotSupported'));
