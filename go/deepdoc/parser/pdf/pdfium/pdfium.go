...
import (
	...
	pdfsync "ragflow/go/deepdoc/parser/pdf/pdfsync"
)
...
func PageSize(pdfData []byte, pageIdx int) (width, height float64, err error) {
	pdfsync.With(func() {
		Init()
		_, _, pw, ph, closeAll, e := openPage(pdfData, pageIdx)
		if e != nil {
			err = e
			return
		}
...
func RenderPage(pdfData []byte, pageIdx int, dpi float64) (out *image.RGBA, err error) {
	pdfsync.With(func() {
		Init()
		_, page, pw, ph, closeAll, e := openPage(pdfData, pageIdx)
		if e != nil {
			err = e
			return
		}
		...
		if bitmap == nil {
			err = fmt.Errorf("pdfium: FPDFBitmap_Create(%d,%d) returned nil", pxW, pxH)
			return
		}
...
// outline trees. pdfium is not thread-safe; all FPDF_* calls are serialized by
// the shared pdfsync.Mu.
