...
import (
	...
	"ragflow/go/harness/core/schema"
)
...
func TestReActAgent_BasicGenerate(t *testing.T) {
	...
	for _, e := range events {
		if e.Output != nil && e.Output.MessageOutput != nil {
			if e.Output.MessageOutput.Message.Content == "Hello!" {
				found = true
			}
		}
	}
...
func TestCallbackFilter_AgentNameMatch(t *testing.T) {
	...
	if len(o.callbacks) == 0 {
		t.Error("callbacks should pass through for matching agent")
	}
}
...
func TestRunOptions(t *testing.T) {
	tests := []struct {
		...
		{"AgentNames", WithAgentNames("a1"), func(t *testing.T, o *runOptions) {
			if len(o.agentNames) != 1 || o.agentNames[0] != "a1" {
				t.Error("agent names not set")
			}
		}},
	}
	for _, tt := range tests {
...
func TestSchemaMessageTypes(t *testing.T) {
	t.Run("UserMessage", func(t *testing.T) {
		...
		if m.Role != schema.RoleUser || m.Content != "Hello" {
...
func TestConcurrentCancel(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(id int) {
			defer wg.Done()
			model := &mockModel{}
			model.addResp(fmt.Sprintf("concurrent-%d", id))
			agent := reActAgentSetup(model, nil)
			agent.name = "cc"
...
func (t *typedAgentMock) Run(ctx context.Context, input *AgentInput, opts ...RunOption) *AsyncIterator[*AgentEvent] {
	return nil
}
...
func TestGetAgentType_Default(t *testing.T) {
	agent := reActAgentSetup(&mockModel{}, nil)
	if gt := getAgentType(agent); gt != "ReActAgent" {
		t.Errorf("expected ReActAgent, got %s", gt)
	}
}
...
func (m *errorMiddleware) BeforeAgent(ctx context.Context, rc *ReActAgentContext) (context.Context, *ReActAgentContext, error) {
	if m.failAt == "BeforeAgent" {
		return ctx, nil, errors.New("error in BeforeAgent")
	}
	return ctx, rc, nil
}
func (m *errorMiddleware) BeforeModelRewrite(ctx context.Context, state *ReActAgentState, mc *ModelContext) (context.Context, *ReActAgentState, error) {
	if m.failAt == "BeforeModelRewrite" {
		return ctx, nil, errors.New("error in BeforeModelRewrite")
	}
	return ctx, state, nil
}
func (m *errorMiddleware) AfterModelRewrite(ctx context.Context, state *ReActAgentState, mc *ModelContext) (context.Context, *ReActAgentState, error) {
	if m.failAt == "AfterModelRewrite" {
		return ctx, nil, errors.New("error in AfterModelRewrite")
	}
	return ctx, state, nil
}
func (m *errorMiddleware) AfterAgent(ctx context.Context, state *ReActAgentState) (context.Context, error) {
	if m.failAt == "AfterAgent" {
		return ctx, errors.New("error in AfterAgent")
	}
	return ctx, nil
}
